...
if __name__ == '__main__':
    ...
    for x, y in constants.xy_pairs:
        ...
        if x != 48:
            df_path = '/scratch/mimiciii/training_data/metavision_sepsis_blood_only_data_08_10_20_sensitivity_' + str(
                x) + '_' + str(y) + '.csv'
        else:
        ...
        for definition in constants.FEATURES:
            ...
            if definition == 't_sofa':
                ages_.append([str(x) + ',' + str(y), definition,
                              "{:.2f}".format(median_age) + ' \pm ' + "{:.2f}".format(iqr_age), \
                              "{:.2f}".format(median_age_non) + ' \pm ' + "{:.2f}".format(iqr_age_non), \
                              "{:.2f}".format(median_age_sep) + ' \pm ' + "{:.2f}".format(iqr_age_sep)])
                los_.append(
                    [str(x) + ',' + str(y), definition, "{:.2f}".format(median_los) + ' \pm' + "{:.2f}".format(iqr_los), \
                     "{:.2f}".format(median_los_non) + ' \pm ' + "{:.2f}".format(iqr_los_non), \
                     "{:.2f}".format(median_los_sep) + ' \pm ' + "{:.2f}".format(iqr_los_sep)])
            else:
                ages_.append(
                    [str(x) + ',' + str(y), definition, "{:.2f}".format(median_age) + ' \pm' + "{:.2f}".format(iqr_age), \
                     '-', \
                     "{:.2f}".format(median_age_sep) + ' \pm ' + "{:.2f}".format(iqr_age_sep)])
                los_.append(
                    [str(x) + ',' + str(y), definition, "{:.2f}".format(median_los) + ' \pm' + "{:.2f}".format(iqr_los), \
        ...
        mort_df = pd.DataFrame(mortalities_,
                               columns=['x,y', 'definition', 'mortality', 'mortality_nonsepsis', 'mortality_sepsis'])
