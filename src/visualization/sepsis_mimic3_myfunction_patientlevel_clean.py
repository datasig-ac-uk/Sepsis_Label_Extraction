...
def probs_extraction(prob_preds,labels,val_full_indices,a1=6):
    ...
    for i in range(k):
        ...
        current_fullindices=[int(tt)+np.arange(current_length_perpatient[j])+current_length_cumsum[j] for j in range(current_patient_num)]
...
def patient_level_performance(val_preds,labels_true,val_full_indices):
    ...
    for i in range(k):
        ...
        current_preds_septic_perpatient=np.array([int(len(np.where(val_preds[current_fullindices[j]]>0)[0])>0) for j in range(current_patient_num)])
        preds_septic_perpatient=np.append(preds_septic_perpatient,current_preds_septic_perpatient)
        
        current_true_septic_perpatient=np.array([int(len(np.where(labels_true[current_fullindices[j]]>0)[0])>0) for j in range(current_patient_num)])        
        true_septic_perpatient=np.append(true_septic_perpatient,current_true_septic_perpatient)        
        
        current_preds_septic_time_perpatient=np.array([np.where(val_preds[current_fullindices[j]]>0)[0][0] for j in range(current_patient_num) \
        ...
        current_true_septic_time_perpatient=np.array([np.where(labels_true[current_fullindices[j]]>0)[0][0] for j in range(current_patient_num) \
                                                      if current_true_septic_perpatient[j]!=0 and\
                                                      len(np.where(val_preds[current_fullindices[j]]>0)[0])!=0])
...
def patient_level_test_performance(test_preds,labels_true,test_full_indices, icuid_seq=None, a1=6):
    ...
    preds_septic_perpatient=np.array([int(len(np.where(test_preds[test_full_indices[j]]>0)[0])>0) for j in range(patient_length)])
 
    true_septic_perpatient=np.array([int(len(np.where(labels_true[test_full_indices[j]]>0)[0])>0) for j in range(patient_length)])             
    preds_septic_time_perpatient=np.array([len(np.where(test_preds[test_full_indices[j]]==0)[0]) for j in range(patient_length)\
                                 if true_septic_perpatient[j]!=0 and len(np.where(test_preds[test_full_indices[j]]>0)[0])!=0])
        
    true_septic_time_perpatient=np.array([len(np.where(labels_true[test_full_indices[j]]==0)[0]) for j in range(patient_length)\
                                 if true_septic_perpatient[j]!=0 and len(np.where(test_preds[test_full_indices[j]]>0)[0])!=0])
