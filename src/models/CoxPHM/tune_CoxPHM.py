...
if __name__ == '__main__':
    ...
    Root_Data, Model_Dir, _, _ = mimic3_myfunc.folders(current_data, model=model)
    ...
    for definition in constants.FEATURES:
        ...
        df = pd.read_pickle(Data_Dir +str(x) + '_' + str(y) + definition[1:] + '_dataframe.pkl')
        features = np.load(Data_Dir + 'james_features'+'_'+str(x) + '_' + str(y) + definition[1:] + '.npy')
        ...
        df_coxph = coxphm_functions.Coxph_df(df, features, coxphm_functions.original_features, T, labels, signature=False)
        ray.init(num_cpus=5)
