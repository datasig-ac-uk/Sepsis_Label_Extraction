...
#     config_dir = constants.MODELS_DIR + 'blood_only_data/LGBM/hyperparameter/config'
    ...
    for x, y in x_y:
        ...
        for a1 in T_list:
            for definition in definitions:
                ...
                preds, prob_preds, auc, specificity, accuracy = lgbm_functions.model_training(
                    model_dir, feature, label)

                mimic3_myfunc.create_folder(Output_predictions+purpose)
                np.save(Output_predictions+purpose + '/prob_preds_' + str(x) +
                        '_' + str(y) + '_' + str(a1) + definition[1:] + '.npy', prob_preds)

                results.append([str(x) + ',' + str(y), a1,
                               definition, auc, specificity, accuracy])
    ...
    results_patient_level_df.to_csv(
        Output_results + 'lgbm_'+purpose+'_patient_level_results.csv')
    ############################################
