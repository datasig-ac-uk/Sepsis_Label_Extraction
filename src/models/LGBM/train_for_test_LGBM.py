...
def train_LGBM(T_list, x_y, definitions, data_folder):
    results = []
    ...
    for x, y in x_y:
        for a1 in T_list:
            for definition in definitions:
                ...
                if  data_folder == constants.exclusion_rules[-1]:
                    with open(Model_Dir_parameter + 'lgbm_best_paras' + definitions[-1][1:] + '.pkl', 'rb') as file:
    ...
    result_df.to_csv(Output_predictions+
                     'train/train_for_test_results.csv')  
