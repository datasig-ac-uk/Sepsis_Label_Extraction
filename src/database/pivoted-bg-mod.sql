...
CREATE MATERIALIZED VIEW pivoted_bg_mod AS
  ...
  WITH i AS
       ...
       (
        ...
        -- Select the relevant data and check they are sensible
       , pvt AS
...
SELECT iid.icustay_id, adm.hadm_id, grp.*
  FROM grp
  LEFT JOIN adm
  ...
  WITH stg_spo2 AS
       ...
       (
        SELECT bg.*
          ...
          -- Find spo2 which occurred at most 6 hours before this blood gas - when checking next measurement of SpO2, we find that there are 3703790, 155319, 19857, 11400, 3098, 2148, 1019, 981, 466 measurements that are 1, 2, 3, 4, 5, 6, 7, 8, 9 hours after the previous reading respectively and we have decided to truncate at 6 hours
