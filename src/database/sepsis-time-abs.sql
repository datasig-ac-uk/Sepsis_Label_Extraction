/* 
This is a variation on the main script to extract sepsis time, sepsis-time-blood.sql. Here instead of finding a change in the SOFA score of 2, we will use the time within the SOFA window where we first observe an absolute value of 2. This is in line with the original Sepsis-3 definition, though we are looking at an ICU cohort so we may expect a large number of patients to come into the ICU with some degree of organ failure.
...
*/
  ...
  DROP MATERIALIZED VIEW IF EXISTS sepsis_cohort_time_blood_abs_2412 CASCADE;
CREATE MATERIALIZED VIEW sepsis_cohort_time_blood_abs_2412 AS
  ...
  WITH serv AS
       ...
       (
        ...
        -- Around each suspicion of infection, check whether the SOFA score exceeds an absolute value of 2
       , sofa_times as
       (
        SELECT u.icustay_id, t_suspicion,  MIN(starttime) AS t_sofa   
          FROM unique_times u
         ...
         WHERE (starttime <= t_suspicion + INTERVAL '12' HOUR 
           ...
           AND sofa_24hours >=2
         GROUP BY u.icustay_id, t_suspicion
        )
