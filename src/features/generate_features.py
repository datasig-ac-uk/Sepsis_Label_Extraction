...
if __name__ == '__main__':
    ...
    for x, y in xy_pairs:
        path_df_train = DATA_DIR + 'raw/further_split/train_' + str(x) + '_' + str(y) + '.csv' if fake_test else \
            DATA_DIR + 'raw/no_gcs_08_10_20_sensitivity_' + str(x) + '_' + str(y) + '.csv'
            #DATA_DIR + 'raw/metavision_sepsis_blood_only_data_08_10_20_sensitivity_' + str(x) + '_' + str(y) + '.csv'

        path_df_test = DATA_DIR + 'raw/further_split/val_' + str(x) + '_' + str(y) + '.csv' if fake_test else \
        DATA_DIR + 'raw/no_gcs_08_10_20_sensitivity_' + str(x) + '_' + str(y) + '.csv'
        ...
        featureset_generator(path_df_train, Save_Dir_train, x=x, y=y, a2=a2, definitions=definitions, T_list=T_list,strict_exclusion=False)
        featureset_generator(path_df_test, Save_Dir_test, x=x, y=y, a2=a2, definitions=definitions, T_list=T_list,strict_exclusion=False)
"""
