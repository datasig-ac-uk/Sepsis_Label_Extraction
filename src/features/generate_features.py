...
if __name__ == '__main__':
    ...
    xy_pairs = [(6,3),(12,6),(24, 12),(48,24)]
    data_folder = 'blood_only'
    fake_test = False
    """
    for x, y in xy_pairs:
        path_df_train = constants.DATA_DIR + 'raw/further_split/train_' + str(x) + '_' + str(y) + '.csv' if fake_test else \
            constants.DATA_DIR + 'raw/metavision_sepsis_blood_only_data_08_10_20_sensitivity_' + str(x) + '_' + str(y) + '.csv'
    ...
    x, y = 24, 12
    data_list = ['no_gcs', 'all_cultures', 'absolute_values', 'strict_exclusion']
    for data_folder in data_list:
        ...
        if fake_test:
            ...
            data_folder1 = 'metavision_sepsis_blood_only_data' if strict_exlucion else data_folder
        path_df_train = constants.DATA_DIR + 'raw/further_split/' + data_folder1 + '_train_' + str(x) + '_' + str(
            y) + '.csv' if fake_test else \
            constants.DATA_DIR + 'raw/' + data_folder1 + '_08_10_20_sensitivity_' + \
            str(x) + '_' + str(y) + '.csv'
        path_df_test = constants.DATA_DIR + 'raw/further_split/' + data_folder1 + '_val_' + str(x) + '_' + str(
            y) + '.csv' if fake_test else \
            constants.DATA_DIR + 'raw/' + data_folder1 + '_08_10_20_sensitivity_' + \
            str(x) + '_' + str(y) + '.csv'
