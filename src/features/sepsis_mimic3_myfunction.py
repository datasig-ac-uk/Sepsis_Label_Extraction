...
def folders(current_data, model='LGBM'):
    ...
    Output_predictions = constants.OUTPUT_DIR + 'predictions/' + current_data + model + '/'
...
def featureset_generator(path_df, Save_Dir, x=24, y=12, a2=0, T_list=constants.T_list,
    ...
    for definition in definitions:
        ...
        np.save(Save_Dir + 'icustay_id' + definition[1:] +'_'+str(x)+'_'+str(y)+'.npy', icuid_sequence)

        print('save ICU lengths for data set')
        icustay_lengths = list(df_sepsis1.groupby('icustay_id').size())
        np.save(Save_Dir + 'icustay_lengths' + definition[1:]  +'_'+str(x)+'_'+str(y)+ '.npy', icustay_lengths)
        ...
        dataset.save(Save_Dir + definition[1:] +'_'+str(x)+'_'+str(y)+ '_ffill.tsd')

        print('gengerate and save labels')
        for T in T_list:
            print('T= ' + str(T))
            labels = labels_generator(df_sepsis1, a1=T, x=x,y=y,Data_Dir=Save_Dir, definition=definition, save=True)
...
def dataframe_cv_pack(df_sepsis, k=5, x=24,y=12,definition='t_sepsis_min', path_save='./', save=True):
    ...
    if not save:
        ...
        shuffled_indices = np.load(path_save + 'shuffled_indices' + definition[1:]+'_'+str(x)+'_'+str(y) + '.npy')
        ...
        np.save(path_save + 'icustay_lengths' + definition[1:] +'_'+str(x)+'_'+str(y)+ '.npy', icustay_lengths)

        shuffled_indices = np.arange(total_indices)
        random.seed(42)
        random.shuffle(shuffled_indices)
        np.save(path_save + 'shuffled_indices' + definition[1:] +'_'+str(x)+'_'+str(y)+ '.npy', shuffled_indices)
...
def labels_generator(df, a1=6, x=24,y=12, Data_Dir='./', definition='t_sepsis_min', save=True):
    ...
    if save:
        np.save(Data_Dir + 'label' + definition[1:] + '_'+str(x)+'_'+str(y)+'_' + str(a1) + '.npy', label)
